<div class="modal fade" id="modalExport" tabindex="0" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        ...
        $.post("<?php echo site_url('activityarchieve/export') ?>", value)
            .done(function(data, status) {
                ...
                if (importType == 'pdf') {
                    ...
                    var columns = ["Nama", "Tempat", "Deskripsi", "Tanggal Kegiatan", "Jumlah Peserta", "Tanggal Input"]
                    var rows = []

                    $.each(response, function(indexRow, row) {
                        var temp = [row['Nama'], row['Tempat'], row['Deskripsi'], row['Tanggal Kegiatan'], row['Jumlah Peserta'], row['Tanggal Input']]
                        rows.push(temp)
                    })
                    ...
                    document.save('Activity Archieve.pdf')

                } else {

                    var fileName = "Activities Report"
